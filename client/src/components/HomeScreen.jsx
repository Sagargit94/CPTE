...
function StatusBadge({ score, status }) {
  if (status === 'in_progress') return (
    <span style={{ display:'inline-flex', alignItems:'center', gap:'0.3rem', padding:'0.2rem 0.7rem', borderRadius:'9999px', fontSize:'0.72rem', fontWeight:'700', background:'var(--success-light)', color:'var(--success)', border:'1px solid var(--success)' }}>
      <span style={{ width:'5px', height:'5px', borderRadius:'50%', background:'var(--success)', display:'inline-block' }} /> IN PROGRESS
    </span>
  );
...
export default function HomeScreen({ user, onStartExam, onDashboard }) {
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg)', fontFamily: 'var(--font-body)' }}>
      ...
      {/* ── Navbar ── */}
      <nav style={{ background: dark ? 'var(--bg-card)' : '#0d1117', height: '60px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 1.75rem', position: 'sticky', top: 0, zIndex: 200, borderBottom: '1px solid rgba(255,255,255,0.06)', boxShadow: 'var(--shadow-sm)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem' }}>
          <div style={{ width: '32px', height: '32px', borderRadius: '8px', background: 'var(--primary)', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 2px 8px rgba(13,92,115,0.4)' }}>
            <Stethoscope size={16} color="#fff" strokeWidth={2.5} />
          </div>
          <span style={{ fontWeight: '800', fontSize: '1rem', color: '#fff', letterSpacing: '-0.02em', fontFamily: 'var(--font-head)' }}>CPTE Prep</span>
          <span style={{ padding: '0.1rem 0.45rem', background: 'rgba(13,92,115,0.4)', border: '1px solid rgba(13,92,115,0.6)', borderRadius: '4px', fontSize: '0.6rem', color: 'var(--primary-mid)', fontWeight: '800', letterSpacing: '0.08em' }}>CAPR</span>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
          <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
            onClick={toggle}
            style={{ width: '34px', height: '34px', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(255,255,255,0.07)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '8px', cursor: 'pointer', color: 'rgba(255,255,255,0.7)' }}>
            {dark ? <Sun size={14} /> : <Moon size={14} />}
          </motion.button>
          <motion.button whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }}
            onClick={onDashboard}
            style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', padding: '0.4rem 0.85rem', background: 'rgba(255,255,255,0.07)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '8px', cursor: 'pointer', color: 'rgba(255,255,255,0.85)', fontSize: '0.8rem', fontWeight: '600', fontFamily: 'var(--font-body)' }}>
            <BarChart2 size={13} /> Analytics
          </motion.button>
          <motion.button whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }}
            onClick={() => supabase.auth.signOut()}
            style={{ display: 'flex', alignItems: 'center', gap: '0.35rem', padding: '0.4rem 0.75rem', background: 'transparent', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '8px', cursor: 'pointer', color: 'rgba(255,255,255,0.45)', fontSize: '0.8rem', fontWeight: '600', fontFamily: 'var(--font-body)' }}>
      ...
      <section style={{ background: dark ? 'linear-gradient(160deg, var(--bg-card) 0%, var(--bg-subtle) 50%, var(--bg-card) 100%)' : 'linear-gradient(160deg, #0d1117 0%, #162035 35%, #0d3347 65%, #0d1117 100%)', padding: '3.5rem 1.5rem', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ maxWidth: '800px', margin: '0 auto', textAlign: 'center', position: 'relative' }}>
          <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}
            style={{ display: 'inline-flex', alignItems: 'center', gap: '0.4rem', background: 'rgba(13,92,115,0.2)', border: '1px solid rgba(13,92,115,0.4)', borderRadius: '9999px', padding: '0.3rem 0.9rem', fontSize: '0.7rem', color: 'var(--primary-mid)', fontWeight: '700', letterSpacing: '0.1em', marginBottom: '1.5rem' }}>
            <Zap size={10} /> CPTE BY CAPR · CANADA
          </motion.div>

          <motion.h1 initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: 0.08 }}
            style={{ fontSize: 'clamp(1.9rem, 5vw, 3rem)', fontWeight: '900', letterSpacing: '-0.04em', lineHeight: 1.05, marginBottom: '1rem', fontFamily: 'var(--font-head)', background: dark ? 'linear-gradient(135deg, var(--text) 30%, var(--primary) 100%)' : 'linear-gradient(135deg, #ffffff 30%, #a8d8e8 70%, var(--primary-mid) 100%)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>
      ...
      <div style={{ maxWidth: '980px', margin: '0 auto', padding: '2.25rem 1.25rem' }}>
        ...
        <motion.div variants={{ show: { transition: { staggerChildren: 0.1 } } }} initial="hidden" animate="show"
          ...
          <motion.div variants={cardVariants} className="card"
            style={{ borderColor: 'var(--primary)', borderWidth: '1.5px', padding: '1.6rem', position: 'relative', overflow: 'hidden', boxShadow: '0 4px 24px rgba(13,92,115,0.12)' }}>
            <div style={{ position: 'absolute', top: '-20px', right: '-20px', width: '100px', height: '100px', background: 'radial-gradient(circle, rgba(13,92,115,0.08) 0%, transparent 70%)', borderRadius: '50%', pointerEvents: 'none' }} />
          ...
          {/* Mock */}
          <motion.div variants={cardVariants} className="card"
            style={{ borderColor: 'var(--accent)', borderWidth: '1.5px', padding: '1.6rem', position: 'relative', overflow: 'hidden', boxShadow: '0 4px 24px rgba(200,146,26,0.1)' }}>
            <div style={{ position: 'absolute', top: '-20px', right: '-20px', width: '100px', height: '100px', background: 'radial-gradient(circle, rgba(200,146,26,0.08) 0%, transparent 70%)', borderRadius: '50%', pointerEvents: 'none' }} />
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.9rem', marginBottom: '1.1rem' }}>
              ...
              <div>
                ...
                <div style={{ fontSize: '0.78rem', color: 'var(--text-muted)', marginTop: '0.1rem', fontWeight: '500' }}>Simulate the real CPTE · 100 min</div>
              </div>
            </div>
            <ul style={{ listStyle: 'none', marginBottom: '1.4rem' }}>
        ...
        <div className="card" style={{ overflow: 'hidden' }}>
          {historyLoading ? (
            ...
            <div key={a.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '1rem 1.4rem', borderBottom: i < recentAttempts.length - 1 ? '1px solid var(--border)' : 'none' }}>
